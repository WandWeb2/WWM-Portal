...
function handleUpdateClient($pdo, $input, $secrets) { $user = verifyAuth($input); if ($user['role'] !== 'admin') sendJson('error', 'Unauthorized'); $pdo->prepare("UPDATE users SET full_name=?, email=?, business_name=?, phone=?, status=? WHERE id=?")->execute([$input['full_name'], $input['email'], $input['business_name'], $input['phone'], $input['status'], (int)$input['client_id']]); syncClientToExternal($pdo, (int)$input['client_id'], $secrets); sendJson('success', 'Updated'); }
function handleClientSelfUpdate($pdo, $input, $secrets) { $user = verifyAuth($input); $uid = $user['uid']; $pdo->prepare("UPDATE users SET full_name=?, business_name=?, phone=?, address=?, website=?, position=? WHERE id=?")->execute([strip_tags($input['full_name']), strip_tags($input['business_name']), strip_tags($input['phone']), strip_tags($input['address']), strip_tags($input['website']), strip_tags($input['position']), $uid]); syncClientToExternal($pdo, $uid, $secrets); sendJson('success', 'Profile Updated'); }
function handleUpdateUserRole($pdo, $i) { $user = verifyAuth($i); if ($user['role'] !== 'admin') sendJson('error', 'Unauthorized'); $newRole = $i['role']; if (!in_array($newRole, ['client', 'partner', 'admin'])) sendJson('error', 'Invalid Role'); $pdo->prepare("UPDATE users SET role = ? WHERE id = ?")->execute([$newRole, (int)$i['client_id']]); sendJson('success', 'Role Updated'); }
function handleAssignClientToPartner($pdo, $i) { $user = verifyAuth($i); if ($user['role'] !== 'admin') sendJson('error', 'Unauthorized'); ensurePartnerSchema($pdo); if ($i['action_type'] === 'remove') { $pdo->prepare("DELETE FROM partner_assignments WHERE partner_id=? AND client_id=?")->execute([(int)$i['partner_id'], (int)$i['client_id']]); sendJson('success', 'Removed'); } else { try { $pdo->prepare("INSERT INTO partner_assignments (partner_id, client_id) VALUES (?, ?)")->execute([(int)$i['partner_id'], (int)$i['client_id']]); sendJson('success', 'Assigned'); } catch (Exception $e) { sendJson('success', 'Already Assigned'); } } }
...
function handleImportCRMClients($pdo, $input, $secrets) { set_time_limit(300); $user = verifyAuth($input); if ($user['role'] !== 'admin') sendJson('error', 'Unauthorized'); ensureUserSchema($pdo); $logs = []; $added = 0; if (!empty($secrets['SWIPEONE_API_KEY'])) { $url = "https://api.swipeone.com/api/workspaces/" . $secrets['SWIPEONE_WORKSPACE_ID'] . "/contacts"; $ch = curl_init($url); curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); curl_setopt($ch, CURLOPT_HTTPHEADER, ["x-api-key: " . $secrets['SWIPEONE_API_KEY'], "Content-Type: application/json"]); $crmRes = json_decode(curl_exec($ch), true); curl_close($ch); if (!empty($crmRes['data'])) { $emails = []; foreach ($crmRes['data'] as $c) { if (!empty($c['email'])) { $emails[] = $c['email']; } } if (!empty($emails)) { $existingEmails = []; $chunks = array_chunk($emails, 900); foreach ($chunks as $chunk) { $placeholders = str_repeat('?,', count($chunk) - 1) . '?'; $stmt = $pdo->prepare("SELECT email FROM users WHERE email IN ($placeholders)"); $stmt->execute($chunk); while ($row = $stmt->fetch()) { $existingEmails[$row['email']] = true; } } $toInsert = []; foreach ($crmRes['data'] as $c) { $email = $c['email'] ?? ''; if (!$email || isset($existingEmails[$email])) continue; $toInsert[] = [$email, trim(($c['firstName']??'').' '.($c['lastName']??''))]; $existingEmails[$email] = true; } if (!empty($toInsert)) { $insertChunks = array_chunk($toInsert, 450); foreach ($insertChunks as $chunk) { $values = []; $params = []; foreach ($chunk as $row) { $values[] = "(?, ?, 'client', 'pending_invite')"; $params[] = $row[0]; $params[] = $row[1]; $logs[] = "[CRM] Imported " . $row[0]; $added++; } $sql = "INSERT INTO users (email, full_name, role, status) VALUES " . implode(", ", $values); $pdo->prepare($sql)->execute($params); } } } } else { $logs[] = "[CRM] No contacts found."; } } else { $logs[] = "[CRM] Skipped (Missing Config)"; } if (!empty($secrets['GOOGLE_REFRESH_TOKEN'])) { $accessToken = getGoogleAccessToken($secrets); if ($accessToken) { $gUrl = "https://people.googleapis.com/v1/people/me/connections?personFields=names,emailAddresses,phoneNumbers,organizations,metadata&pageSize=100"; $ch = curl_init($gUrl); curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer $accessToken", "Accept: application/json"]); $gRes = json_decode(curl_exec($ch), true); curl_close($ch); if (!empty($gRes['connections'])) { foreach ($gRes['connections'] as $person) { $email = $person['emailAddresses'][0]['value'] ?? ''; if (!$email) continue; $name = $person['names'][0]['displayName'] ?? 'Google Client'; $googleId = $person['resourceName'] ?? ''; $stmt = $pdo->prepare("SELECT id, google_resource_name FROM users WHERE email = ?"); $stmt->execute([$email]); $existing = $stmt->fetch(); if ($existing) { if(empty($existing['google_resource_name'])) { $pdo->prepare("UPDATE users SET google_resource_name = ? WHERE id = ?")->execute([$googleId, $existing['id']]); $logs[] = "[GOOGLE] Linked ID for $email"; } } else { $pdo->prepare("INSERT INTO users (email, full_name, google_resource_name, role, status) VALUES (?, ?, ?, 'client', 'pending_invite')")->execute([$email, $name, $googleId]); $logs[] = "[GOOGLE] Imported $email"; $added++; } } } else { $logs[] = "[GOOGLE] No connections found."; } } else { $logs[] = "[GOOGLE] Auth failed."; } } else { $logs[] = "[GOOGLE] Skipped."; } sendJson('success', "Sync Cycle Complete", ['logs' => $logs]); }
function handleImportStripeClients($pdo, $input, $secrets) {
    set_time_limit(300);
    $user = verifyAuth($input);
    if ($user['role'] !== 'admin') sendJson('error', 'Unauthorized');
    ensureUserSchema($pdo);

    $response = stripeRequest($secrets, 'GET', 'customers?limit=100');
    $customers = $response['data'] ?? [];
    $logs = [];

    // Extract all emails
    $emails = array_filter(array_column($customers, 'email'));
    $existingUsers = [];

    if (!empty($emails)) {
        // Chunking to avoid SQLite param limits
        $chunks = array_chunk($emails, 900);
        foreach ($chunks as $chunk) {
            $placeholders = implode(',', array_fill(0, count($chunk), '?'));
            $s = $pdo->prepare("SELECT email, stripe_id FROM users WHERE email IN ($placeholders)");
            $s->execute($chunk);
            while ($row = $s->fetch()) {
                $existingUsers[$row['email']] = $row['stripe_id'];
            }
        }
    }
    ...
    $toInsert = [];
    $toInsertParams = [];
    $updateStmt = $pdo->prepare("UPDATE users SET stripe_id = ? WHERE email = ?");
    ...
    foreach ($customers as $c) {
        ...
        if (array_key_exists($email, $existingUsers)) {
            if (empty($existingUsers[$email])) {
                $updateStmt->execute([$c['id'], $email]);
                // Update our cache so we don't try to link it again if duplicated
                $existingUsers[$email] = $c['id'];
                $logs[] = "[STRIPE] Linked ID for $email";
            }
        } else {
            $toInsert[] = "(?, ?, ?, 'client', 'active')";
            $toInsertParams[] = $email;
            $toInsertParams[] = $c['name'];
            $toInsertParams[] = $c['id'];
            // Mark as existing so duplicates in the same batch aren't inserted twice
            $existingUsers[$email] = $c['id'];
            $logs[] = "[STRIPE] Imported $email";
        }
    }

    if (!empty($toInsert)) {
        // SQLite limit is 999 vars per query, we use 3 vars per insert -> max 333 rows per chunk
        $chunkSize = 300;
        $insertChunks = array_chunk($toInsert, $chunkSize);
        $paramChunks = array_chunk($toInsertParams, $chunkSize * 3);

        foreach ($insertChunks as $idx => $chunk) {
            $sql = "INSERT INTO users (email, full_name, stripe_id, role, status) VALUES " . implode(", ", $chunk);
            $pdo->prepare($sql)->execute($paramChunks[$idx]);
        }
    }

    sendJson('success', "Stripe Sync Complete", ['logs' => $logs]);
}
