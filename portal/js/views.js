...
window.ProjectsView = ({ token, role, currentUserId }) => {
    ...
    React.useEffect(() => { window.safeFetch(API_URL, { method: 'POST', body: JSON.stringify({ action: 'get_projects', token }) }).then(r => setProjects(r.projects||[])); }, [token]);
    if(active) return <TaskManager project={active} token={token} onClose={()=>setActive(null)} />;
    return (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">{projects.map(p=><ProjectCard key={p.id} project={p} isAdmin={role==='admin'} setActiveProject={setActive} />)}</div>);
};
